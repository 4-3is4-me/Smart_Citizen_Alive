...
void drawmoon() {
  ...
  ellipse(moonX, moonY, height/moonsize, height/moonsize);
  fill(skycol);
...
void drawhill(int[] ys, int red, int green, int blue) {
  ...
  curveVertex(-(width/numberofvertex), height);
  for (int i = 0; i < numberofvertex; i++) {
    float yadj = map(noise(xup, yup), 0, 1, ys[i], ys[i]+40);
    curveVertex(xpointsarr[i], yadj);
    xup += 0.5;
    yup += 0.5;
  }
  curveVertex(width+(width/numberofvertex), height);  //last is to be duplicated too
...
void drawrain() {
  ...
  for (int i = 0; i < rain; i++) {
    ...
    float rL = rY+random(rain*2);
    stroke(random(255), random(255));
    // if it's noisy - implies windy so make rain sideways...
    if ( sensorlatst.get(str(sensornums.get("Noise"))) < 46) {
      line(rX, rY, rX, rL);
    } else {
      line(rX, rY, rX-10, rL);
    }
